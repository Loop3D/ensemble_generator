...
def parse_gm_task(model_task):
    # taskfile_path = path+root_name+'.task'
    #tasks = open('project_export.task', "r")
    path = pathlib.PurePosixPath(model_task)
    tasks = open(model_task, "r")
    contents = tasks.readlines()
    tasks.close()

    #%% Check model directory structure

    if not os.path.exists(path.parent / "./output"):
        os.makedirs(path.parent / "./output")
    ...
    allmeta = open(path.name + '_metadata.csv', "w")
    ...
    allp = open(path.name + '_petrophsyics.csv', "w")
